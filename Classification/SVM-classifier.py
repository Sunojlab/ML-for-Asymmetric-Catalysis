...
#the following code is for Support Vector Machine Classifier with k-fold cv for 100 different seed values and reports the average rmse along with standard deviation
...
best_gamma_values = 1.0*np.arange(0,len(seeds),1)

gamma_values = [0.00001, 0.0001, 0.001, 0.01, 0.1, 1] #np.arange(100,200,100) 

seed_numest_pair = []

for seed in seeds:
    for gamma_val in gamma_values:
        seed_numest_pair.append([seed,gamma_val])
...
estind_values = np.arange(len(gamma_values))
...
def kfoldcv(seed,gammaparam):
    ...
    for i in np.arange(len(splitpoints)):
        ...
        clf = SVC(gamma=gamma_val_arg, kernel='rbf', C=10)
    ...
    return seed,gamma_val_arg,time.time() - start,avg_accuracy


def compute_testrmse(seed,best_gamma_val):
    start = time.time()
    ...
    final_best_gamma_val = best_gamma_val
    final_clf = SVC(gamma=final_best_gamma_val, kernel='rbf', C=10)
    ...
    return seed,best_gamma_val,time.time() - start, final_accuracy, final_tr_accuracy
...
def main():
    ...
    for seed in seeds: 
        avg_rmse_ret.append([])
        gamma_val_ret.append([])    
    
    start = time.time()
    
    with concurrent.futures.ProcessPoolExecutor() as executor:
         for seed,gamma_val,time_ret,avg_accuracy in executor.map(kfoldcv, seed_numest_pair_arr[:,0], seed_numest_pair_arr[:,1]):
             seed_index = int((seed-seed_start)/seed_step)
             avg_rmse_ret[seed_index].append(avg_accuracy)
             gamma_val_ret[seed_index].append(gamma_val)
             print('seed:%f numest: %f time: %f avg_rmse: %f' %(seed, gamma_val,time_ret,avg_accuracy), flush=True)
    ...
    for seed in seeds: 
        ...
        estlist = gamma_val_ret[seed_index]
        #print(estlist)
        #print(estlist[argminind])
        best_gamma_values[seed_index]= (estlist[np.argmax(tmp)])
        
        print('seed:%d argmin numest: %f' %(seed,best_gamma_values[seed_index]), flush=True)

    #print(best_est)

    start = time.time()        
    with concurrent.futures.ProcessPoolExecutor() as executor:
         for seed,best_gamma_val,time_ret,test_rmse,train_rmse in executor.map(compute_testrmse, seeds, best_gamma_values):
             print('seed:%f best C val:%f time: %f test_rmse: %f train rmse: %f' %(seed,best_gamma_val,time_ret,test_rmse, train_rmse), flush=True)
             seed_index = int((seed-seed_start)/seed_step)
