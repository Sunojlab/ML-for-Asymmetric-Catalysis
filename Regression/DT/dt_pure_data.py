...
#the following code is for Decision tree regression with k-fold cv for 100 different seed values and reports the average rmse along with standard deviation
...
seed_start = 0
seed_end = 10000 #this depends on the number of seeds
seed_step = 100
...
def kfoldcv(seed,numest):
    ...
    for i in np.arange(len(splitpoints)):
        ...
        regr = DecisionTreeRegressor(criterion='mse', splitter='best', max_depth=num_estimators_arg, min_samples_split=2, 
...
def compute_testrmse(seed,best_numest):
    ...
    final_regr = DecisionTreeRegressor(criterion='mse', splitter='best', max_depth=final_best_estimator, min_samples_split=2, 
                                     min_samples_leaf=1, min_weight_fraction_leaf=0.0, max_features=None, 
                                     random_state=None, max_leaf_nodes=int(math.sqrt(len(final_train_feat[0]))), min_impurity_decrease=0.0, 
                                     min_impurity_split=None, presort=False)
...
def main():
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
         for seed,bestest,time_ret,test_rmse,train_rmse in executor.map(compute_testrmse, seeds, best_est):
             print('seed:%f bestest:%f time: %f test_rmse: %f train rmse: %f' %(seed,bestest,time_ret,test_rmse, train_rmse), flush=True)
             seed_index = int((seed-seed_start)/seed_step)
