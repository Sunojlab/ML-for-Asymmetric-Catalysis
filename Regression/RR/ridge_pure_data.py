...
#the following code is for ridge regression with k-fold cv for 100 different seed values and reports the average rmse along with standard deviation
...
num_estimators_values = np.arange(1,101,1) 
...
def kfoldcv(seed,numest):
    ...
    for i in np.arange(len(splitpoints)):
        ...
        regr = linear_model.Ridge(alpha=num_estimators_arg)
...
def compute_testrmse(seed,best_numest):
    ...
    final_regr = linear_model.Ridge(alpha=final_best_estimator)
