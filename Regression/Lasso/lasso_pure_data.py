...
#the following code is for Lasso regression with k-fold cv for 100 different seed values and reports the average rmse along with standard deviation
...
seed_start = 0.0
seed_end = 10000.0 #this depends on the number of seeds
seed_step = 100.0
...
num_estimators_values = 1.0*np.arange(1, 101, 10) 
...
def kfoldcv(seed,numest):
    ...
    for i in np.arange(len(splitpoints)):
        ...
        regr = ElasticNet(alpha = num_estimators_arg, l1_ratio=1, random_state=0)
...
def compute_testrmse(seed,best_numest):
    ...
    final_regr = ElasticNet(alpha = final_best_estimator, l1_ratio=1, random_state=0)
