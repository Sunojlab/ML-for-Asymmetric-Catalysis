...
#the following code is for support vector regression (SVR) with k-fold cv for 100 different seed values and reports the average rmse along with standard deviation
...
for seed in seeds:
    for C_val in C_values:
        seed_numest_pair.append([seed,C_val])
...
def kfoldcv(seed,numest):
    ...
    for i in np.arange(len(splitpoints)):
        ...
        regr = SVR(C=C_val_arg, cache_size=200, coef0=0.0, degree=3, epsilon=0.01, gamma='auto',
    ...
    return seed,C_val_arg,time.time() - start,avg_rmse


def compute_testrmse(seed,best_C_val):
    start = time.time()
    ...
    final_regr = SVR(C=final_best_C_val, cache_size=200, coef0=0.0, degree=3, epsilon=0.2, gamma=0.1,
                          kernel='rbf', max_iter=-1, shrinking=True, tol=0.001, verbose=False)
    ...
    return seed,best_C_val,time.time() - start, final_rmse, final_tr_rmse
...
def main():
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
         for seed,C_val,time_ret,avg_rmse in executor.map(kfoldcv, seed_numest_pair_arr[:,0], seed_numest_pair_arr[:,1]):
             seed_index = int((seed-seed_start)/seed_step)
             avg_rmse_ret[seed_index].append(avg_rmse)
             C_val_ret[seed_index].append(C_val)
             print('seed:%f numest: %f time: %f avg_rmse: %f' %(seed, C_val,time_ret,avg_rmse), flush=True)
    ...
    for seed in seeds: 
        ...
        print('seed:%d argmin numest: %f' %(seed,best_C_values[seed_index]), flush=True)

    #print(best_est)

    start = time.time()        
    with concurrent.futures.ProcessPoolExecutor() as executor:
         for seed,best_C_val,time_ret,test_rmse,train_rmse in executor.map(compute_testrmse, seeds, best_C_values):
             print('seed:%f best C val:%f time: %f test_rmse: %f train rmse: %f' %(seed,best_C_val,time_ret,test_rmse, train_rmse), flush=True)
             seed_index = int((seed-seed_start)/seed_step)
