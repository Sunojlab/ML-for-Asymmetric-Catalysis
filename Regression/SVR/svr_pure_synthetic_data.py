...
def compute_testrmse(seed,best_C_val):
    ...
    return seed,best_C_val,time.time() - start, final_rmse, tr_final_rmse
...
def main():
    ...
    for seed in seeds: 
        ...
        print('seed:%d argmin numest: %d' %(seed,best_C_values[seed_index]), flush=True)
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
         for seed,best_C_val,time_ret,test_rmse,train_rmse in executor.map(compute_testrmse, seeds, best_C_values):
             print('seed:%f bestest:%f time: %f test_rmse: %f train_rmse: %f' %(seed,best_C_val,time_ret,test_rmse,train_rmse),flush=True)
             seed_index = int((seed-seed_start)/seed_step)
