...
#the following code is for kernel ridge Regression with k-fold cv for 100 different seed values and reports the average rmse along with standard deviation
...
num_estimators_values = np.arange(100,1001,100) 
...
def kfoldcv(seed,numest):
    ...
    for i in np.arange(len(splitpoints)):
        ...
        regr = KernelRidge(alpha=num_estimators_arg)
...
def compute_testrmse(seed,best_numest):
    ...
    final_regr = KernelRidge(alpha=final_best_estimator)
